import { ChevronLeft, ChevronRight, Bell, User, LogOut, Upload, Settings, Moon, Heart, Search as SearchIcon } from 'lucide-react';
          ...
          <>
            ...
            <div className="relative" ref={profileRef}>
              <button 
                ...
                className="w-10 h-10 rounded-full liquid-glass flex items-center justify-center overflow-hidden hover:scale-105 transition-transform p-0.5"
              >
                {user.photoURL ? (
                  <img src={user.photoURL} alt={user.displayName || 'User'} className="w-full h-full rounded-full object-cover" />
                ) : (
                  <User size={20} className="text-spotify-text-muted" />
                )}
              </button>
              ...
              <AnimatePresence>
                {isProfileOpen && (
                  <motion.div 
                    ...
                    animate={{ opacity: 1, scale: 1, y: 0 }}
                    exit={{ opacity: 0, scale: 0.95, y: 10 }}
                    className="absolute right-0 top-14 w-56 liquid-glass rounded-2xl p-2 z-50"
                  >
                    <div className="px-4 py-3 mb-2 border-b border-white/5">
                      <p className="text-xs font-bold text-spotify-text truncate">{user.displayName || 'User'}</p>
