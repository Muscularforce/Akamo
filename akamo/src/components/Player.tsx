...
const ScrollingText = ({ text, className, speed = 30 }: { text: string; className: string; speed?: number }) => {
  ...
  return (
    <div ref={containerRef} className={`${className} overflow-hidden whitespace-nowrap relative`}>
      <motion.div
        animate={shouldScroll ? { x: [0, -textRef.current!.offsetWidth - 20] } : { x: 0 }}
        transition={{
          duration: shouldScroll ? (textRef.current!.offsetWidth / speed) : 0,
          repeat: Infinity,
          ease: "linear",
          repeatDelay: 2
        }}
...
export default function Player({ currentTrack, isPlaying, onTogglePlay, onNext, onPrev }: PlayerProps) {
  ...
  return (
    <div className="fixed bottom-6 left-1/2 -translate-x-1/2 w-[95%] max-w-6xl z-50">
      <div className="liquid-glass rounded-[2.5rem] px-8 h-24 flex items-center justify-between gap-8 relative overflow-hidden group/player">
        {/* Glow behind cover */}
        <div className="absolute left-0 top-0 w-40 h-full bg-spotify-green/10 blur-3xl pointer-events-none" />
        
        {/* Track Info */}
        <div className="flex items-center gap-5 w-[30%] min-w-[240px] relative z-10">
          <AnimatePresence mode="wait">
            {currentTrack ? (
              ...
              >
                <div className="w-16 h-16 rounded-2xl overflow-hidden shadow-[0_10px_30px_rgba(0,0,0,0.5)] flex-shrink-0 group-hover/player:scale-105 transition-transform duration-500">
                ...
                <div className="flex flex-col flex-1 min-w-0">
                  ...
                  <ScrollingText 
                    ...
                    className="text-xs text-spotify-text-muted hover:text-spotify-text transition-colors cursor-pointer opacity-70 tracking-wide font-medium" 
                    speed={20}
                  />
                </div>
              </motion.div>
        ...
        {/* Volume & Extras */}
        <div className="flex items-center justify-end gap-5 w-[30%] min-w-[200px] relative z-10">
